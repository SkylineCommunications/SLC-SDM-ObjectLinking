namespace Skyline.DataMiner.SDM.ObjectLinking.Tests
{
	...
	[TestClass]
	public class GetLinksByEntityTests
	{
		...
		[TestMethod]
		public void ObjectLinker_GetLinksByEntity_Symmetric()
		{
			...
			var act = () => result = linker.GetLinksByEntity(entity1);

			act.Should().NotThrow();
			result.Should().HaveCount(2);
			result.Should().Contain(link => link.EntityDescriptors.Contains(entity2));
			result.Should().Contain(link => link.EntityDescriptors.Contains(entity3));
		}

		[TestMethod]
		public void ObjectLinker_GetLinksByEntity_Forward()
		{
			var linker = new ObjectLinker(_connection);
			...
			result.Should().Contain(link => link.EntityDescriptors.Contains(entity2));
			result.Should().NotContain(link => link.EntityDescriptors.Contains(entity3));
		}

		[TestMethod]
		public void ObjectLinker_GetLinksByEntity_Backward()
		{
			var linker = new ObjectLinker(_connection);
			...
			act.Should().NotThrow();
			result.Should().HaveCount(1);
			result.Should().NotContain(link => link.EntityDescriptors.Contains(entity2));
			result.Should().Contain(link => link.EntityDescriptors.Contains(entity3));
		}
	}
}